...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/06 09:10:26 by aarid             #+#    #+#             */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	reminder[fd] = read_and_concatenate(fd, reminder[fd]);
	if (!reminder[fd])
		return (NULL);
	line = extract_line(reminder[fd]);
	if (!line)
	{
		free(reminder[fd]);
		return (NULL);
	}
	reminder[fd] = cleaning_reminder(reminder[fd]);
	return (line);
}
